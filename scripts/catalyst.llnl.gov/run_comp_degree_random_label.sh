...
for i in 0 1 2 3; do
    ...
    for pattern in ${patterns[*]}; do
        for flags_count in 0 2 4 8 16 32 64 128 256 512 1024; do
            echo $flags_count
            sbatch -N${nodes[i]}\
                "--export=ARG=-i /dev/shm/rmat${scales[i]} -b /p/lustre1/an4/graph/rmat${scales[i]} -u $flags_count -p $pattern -o ./usr/results,P=$program/run_pattern_matching_beta_1.1" pattern_matching.sbatch
        done
    done
done
