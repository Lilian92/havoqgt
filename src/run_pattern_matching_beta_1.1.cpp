  ...
  if (vertex_metadata_input.size() > 0) {
      ...
      if (mpi_rank == 0) {
          ...
          if (uniform_random_vertex_metadata == 0) {
              ...
              std::cout << "meta data is uniform random generated from 0 to " << uniform_random_vertex_metadata << std::endl;
          }

          for (size_t i=0; i<histogram.size(); i++)
              std::cout << "label " << i << " : " << histogram_global[i] << ", " << histogram_global[i]/(double)vertices_size << "%" << std::endl;
      }
  }
#endif
