...
#ifdef HISTOGRAM_LABEL
  if (vertex_metadata_input.size() > 0) {
      if (mpi_rank == 0)
          std::cout << "meta data from file, not support yet" << std::endl;
  } else {
      size_t labels_size = uniform_random_vertex_metadata;
      size_t vertices_size = graph->max_global_vertex_id();
      if (uniform_random_vertex_metadata == 0) {
          labels_size = log2(vertices_size);
      }
      std::vector<size_t> histogram(labels_size, 0);

      for (vitr_type vitr = graph->vertices_begin(); vitr != graph->vertices_end();
              ++vitr) {
          vloc_type vertex = *vitr;
          histogram[vertex_metadata[vertex]]++;
      }
      ...
      std::vector<size_t> histogram_global(labels_size, 0);
      CHK_MPI(MPI_Allreduce(histogram.data(), histogram_global.data(), labels_size, MPI_LONG,
                  MPI_SUM, MPI_COMM_WORLD));

      //Print
      if (mpi_rank == 0) {
          std::cout << "Graph meta data statics : " << std::endl;
          std::cout << "vertices size : " << vertices_size << std::endl;
          if (uniform_random_vertex_metadata == 0) {
              std::cout << "meta data is generated by log2(vertex degree)" << std::endl;
          } else {
              std::cout << "meta data is uniform random generated from 0 to " << uniform_random_vertex_metadata - 1 << std::endl;
          }

          for (size_t i=0; i<histogram.size(); i++) {
              std::cout << "label " << i << " : " << histogram_global[i] << ", " << histogram_global[i]/(double)vertices_size * 100 << " %" << std::endl;
          }
      }
  }
#endif
