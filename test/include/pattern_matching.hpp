...
size_t pattern_matching_prunejuice(graph_type * graph,
        ...
        bool  enable_edge_temporal_matching = false,
...
typedef struct csr {
    ...
    //edge list and data
    std::vector<size_t> vertex_edgelist_start_opt;
    std::vector<size_t> edges;
    std::vector<EdgeData> edge_data;

    csr() : vertex_num(0),
            edge_num(0),
            vertex_edgelist_start_opt(),
            vertex_data(),
            edges(),
            edge_data() {}
    ...
    void clear() {
        vertex_num = 0;
        edge_num = 0;
        vertex_edgelist_start_opt.clear();
        vertex_data.clear();
        edges.clear();
        edge_data.clear();
    }
...
typedef struct csr_sort_by_vertex_label {
    ...
    std::vector<size_t> each_label_start_index;

    //edge list and data
    std::vector<size_t> vertex_edgelist_start_index;
    std::vector<Vertex> edges;
    std::vector<EdgeData> edge_data;

    //start_pos of each label for each vertex
    std::vector<size_t> start_pos_for_each_label;

    csr_sort_by_vertex_label() : vertex_num(0),
            edge_num(0),
            to_vertex_data(),
            to_vertex_data_id(),
            each_label_start_index(),
            vertex_edgelist_start_opt(),
            edges(),
            edge_data() {}
    ...
    VertexData vertex_data(Vertex id) {
        size_t begin = 0; 
        size_t end = each_label_start_index.size() - 1;
        assert(id>=0 && id<each_label_start_index[end]);
        size_t pos;
        assert(binary_find(each_label_start_index, id, begin, end, pos), "wrong vertex id");
        return to_vertex_data[pos];
    }

    size_t to_vertex_data_id(VertexData data) {
        auto it = to_vertex_data_id.find(data);
        assert (it != to_vertex_data_id.end(), "not valid vertex data input for start end vertex id checking");
        return it -> second;
    }

    void start_end_vertex_id(VertexData data, size_t & start, size_t & end) {
        size_t vertex_data_id = to_vertex_id(data);
        start = each_label_start_index[vertex_data_id];
        end = each_label_start_index[vertex_data_id+1];
    }
    ...
    void init_vertex_label_org(LABEL_SUMMARY & ls) {
        ...
        for (auto it = ls.unique_vertex_data.begin();
                it<ls.unique_vertex_data.end(); it++) {
            to_vertex_data.push_back(*it);
            to_vertex_data_id.push_back(std::make_pair(*it, vertex_data_id));
            vertex_data_id++;
        }
    }
    ...
    bool find_neigbors_with_given_vertex_data(Vertex cur_vertex, VertexData vertex_data,
        ...
        size_t edge_list_begin = vertex_edgelist_start_index[cur_vertex];
        size_t edge_list_end = vertex_edgelist_start_index[cur_vertex + 1];

        if (edge_list_end - edge_list_begin < 1) {
            return false;
        }

        size_t seq_start = (vertex_label_type_num - 1) * cur_vertex;
        size_t vertex_data_id = to_vertex_id(vertex_data);
        if (vertex_data_id == 0) {
            start_index = edge_list_begin;
        } else
            start_index = start_pos_for_each_label[seq_start + vertex_data_id - 1];

        if (vertex_data_id == vertex_label_type_num - 1) {
            end_index = edge_list_end;
        } else
            end_index = start_pos_for_each_label[seq_start + vertex_data_id];
...
typede struct seq_temporal_constraints{
    ...
    seq_temporal_constraints (
            ...
            bool enable_temporal_matching) :
        smaller_than_constraints(), bigger_than_constraints() {
            if (!enable_temporal_matching)
                return;
    ...
    //as ID
    bool pass_temporal_checking(std::stack<EdgeData> & mapped_edge_data,
        ...
        for (auto item : smaller_than_constrains[current_edge_id_in_pattern]) {
        ...
        for (auto item : bigger_than_constrains[current_edge_id_in_pattern]) {
            assert(item != current_edge_id_in_pattern);
            if (item > current_edge_id_in_pattern)
                break;
    ...
    bool pass_temporal_checking(std::stack<EdgeData> & mapped_edge_data,
        ...
        size_t stored_edge_data_size =  = mapped_edge_data.size();
        size_t index = 0;
        for (; index < smaller_than_constrains[current_edge_id_in_pattern].size(); index++) {
            auto item = smaller_than_constrains[current_edge_id_in_pattern][index];
            assert(item != current_edge_id_in_pattern);
            if (item > stored_edge_data_size)
                break;
        ...
        //current edge should bigger than edge_data[item]
        index = 0;
        for (; index < bigger_than_constrains[current_edge_id_in_pattern].size(); index++) {
            auto item = bigger_than_constrains[current_edge_id_in_pattern][index];
            assert(item != current_edge_id_in_pattern);
            if (item > stored_edge_data_size)
                break;
...
size_t pattern_matching_seq(const std::vector<std::tuple<uint64_t, uint64_t, EdgeData>> edge_list,
    ...
    PatternGraph pattern(
            ...
            pattern_input_filename + "_stat",
            false, false);
    PatternTemporalConstraint temporal_constraints(pattern_graph,
                        pattern_dir + "/pattern_temporal_constraint", enable_edge_temporal_matching);
    PatternTemporalConstrainSeq temporal_constraints_seq(temporal_constraints, enable_edge_temporal_matching);
    ...
    //TODO: optimize
    std::vector<bool> active_edges(graph.vertex_num, false);
    size_t num_of_patterns_found = 0;
    std::stack<Vertex> mapping;
    std::stack<EdgeData> mapping_edge_data;
    recurrence_pattern_matching(mapping, mapped_edge_data, number_of_patterns_found,
            ...
            enable_edge_matching, enable_edge_temporal_matching);
    //search matched pattern in a recurrence way and active vertices and
    //edges involved matches
    return 0;
}

void recurrence_pattern_matching (std::stack<Vertex> mapping,
        std::stack<EdgeData> mapped_edge_data, size_t & number_of_patterns_found,
    ...
    size_t pattern_neighbor_list_start = pattern.vertices[cur_vertex];
    size_t pattern_neighbor_list_end = pattern.vertices[cur_vertex + 1];
    size_t pattern_cur_index = pattern_neighbor_list_start;
    std::vector<Vector> vertices_connected;
    while ((pattern_cur_index < pattern_neighbor_list_end) && pattern.edges[pattern_cur_index] < cur_vertex) {
        vertices_connected.push_back(mapping[pattern.edges[pattern_cur_index]]);
        pattern_cur_index++;
    }
    size_t connected_vertices_num = vertices_connected.size();

    //Collect connected vertices' neighbor index with given vertex data
    std::vector<std::pair<size_t, size_t>> start_end_index;
    for (size_t i=0; i<connected_vertices_num; i++) {
        size_t start_index, end_index;
        if (!find_neigbors_with_given_vertex_data(vertices_connected[i],
                    pattern.vertex_data[cur_vertex], start_index, end_index))
            //connected vertex doesn't have
            //neighbors with required vertex data.
            return ;
        start_end_index.push_back(std::make_pair(start_index, end_index));
    }
    ...
    if (connected_vertices_num <= 1) {
        ...
        size_t start_index, end_index;
        if (connected_vertices_num == 1) {
            start_index = std::get<0>(start_end_index[0]);
            end_index = std::get<1>(start_end_index[0]);
        } else {
            graph.start_end_vertex_id(pattern.vertex_data[cur_vertex],
        ...
        for (size_t i=start_index; i<end_index; i++) {
            ...
            if (connected_vertices_num == 1 &&
                ...
                if (enable_edge_matching) {
                    if (!(pattern.edge_data[pattern_neighbor_list_start] == edata))
                        continue;
                }
        ...
        for (size_t i=0; i<connected_vertices_num; i++) {
            size_t count = std::get<1>(start_end_index[i]) - std::get<o>(start_end_index[i]);
            neighbors_count.push_back(std::make_pair(count, i));
        }
        sort(neigbors_count.begin(), neighbors_count.end());
        for (size_t i=0; i<connected_vertices_num; i++) {
            size_t org = std::get<1>(neigbors_count[i]);
            to_new_order[org] = i;
            to_org_order[i] = org;
        }
        //reorder so to reduce binary search amount
        ...
        std::vector<Vector> possible_next_vertices;
        ...
        for (size_t cur_order = 2; cur_order < connected_vertices_num; cur_order++) {
            size_t cur_start_index = std::get<0>(start_end_index[to_org_order[cur_order]]);
            size_t cur_end_index = std::get<0>(start_end_index[to_org_order[cur_order]]);
            for (size_t i=0; i<possible_next_vertices_num; i++) {
                if (pass_checking[i] == true) {
                    Vertex possible_vertex = possible_next_vertices[i];
                    size_t pos;
                    if (!binary_find(graph.edges, possible_vertex, second_start, second_end, pos)) {
                    ...
                    cur_start_index = pos + 1;
                    if (cur_end_index - cur_start_index <= 1)
                        break;
                }
            }
        }
            ...
            if (pass_checking[i] == true) {
                ...
                if (enable_temporal_matching) {
                    ...
                    for (size_t j=0; j<connected_vertices_num; j++) {
                        mappted_edge_data[num_mapped_edges + to_org_order[j]] = possible_next_vertices_edgedata[i][j];
                    }
                }
                recurrence_pattern_matching(mapping, mapped_edge_data, number_of_patterns_found,
                        graph, pattern, temporal_constraints_seq, active_vertices, active_edges);
...
void binary_search_set_intersection_with_constraints(
        ...
        CSR_SORT_BY_VERTEX_LABEL & graph,
        PatternGraph & pattern,
        size_t pattern_neighbor_list_start,
        std::stack<EdgeData> mapped_edge_data,
        PatternTemporalConstraintSeq & temporal_constraints_seq,
        bool enable_edge_matching = false,
        bool enable_edge_temporal_matching = false) {
    ...
    size_t first_start = std::get<0>(start_end_index[first]);
    size_t first_end = std::get<1>(start_end_index[first]);
    size_t second_start = std::get<0>(start_end_index[second]);
    size_t second_end = std::get<1>(start_end_index[second]);
    for (size_t i=first_start; i<first_end; i++) {
        if (enable_edge_matching || enable_edge_temporal_matching) {
            EdgeData edata = graph.edge_data[i];
            if (enable_edge_matching) {
                if (!(pattern.edge_data[pattern_neighbor_list_start + first] == edata))
                    continue;
            }
            if (enable_edge_temporal_matching) {
                if (!temporal_constraints_seq.pass_temporal_checking(mapped_edge_data, edata, pattern_neighbor_list_start + first))
                    continue;
            }
        }
        size_t pos;
        Vertex neig = graph.edges[i];
        if (binary_find(graph.edges, neig, second_start, second_end, pos)) {
...
bool binary_find(std::vector<DataType> & array, DataType element, size_t start, size_t end, size_t & pos) {
    if (end - begin <= 1)
        return false;
...
bool set_edges_active(std::vector<boo> & active_edges,
        CSR_SORT_BY_VERTEX_LABEL & graph,
        Vertex src, std::vector<Vector> & active_neighbors) {
    sort(active_neighbors.begin(), active_neighbors.end());
    size_t begin = graph.vertex_edgelist_start_index[src];
    size_t end = graph.vertex_edgelist_start_index[src+1];
    size_t cur = (begin + end) / 2;
    size_t count = 0;
    while (count < active_neighbors.size()) {
        while (begin < end) {
            if (graph.edges[cur] ==  active_neighbors[count]) {
                active_neighbors[cur] = true;
                begin = cur + 1;
                end = graph.vertex_edgelist_start_index[src+1];
                break;
            }
            if (graph.edges[cur] > active_neighbors[count]) {
                end = cur;
...
void analysis_pattern(PatternGraph & pattern, LABEL_SUMMARY & ls, bool enable_edge_matching = false) {
    ...
    for (size_t v=0; v<pattern.vertex_count; v++) {
        ...
        if (ls.unique_vertex_data.find(cur) == ls.unique_vertex_data.end()) {
            ...
            ls.to_vertex_data.push_back(cur);
            count++;
        } else {
...
bool read_vertex_metadata(const std::string vertex_metadata_filename, CSR_SORT_BY_VERTEX_LABEL & graph,
    ...
    assert(graph.each_label_start_index.size() == 0);
    for (size_t i=0; i<ls.unique_vertex_data.size(); i++) {
        if (each_label_vertex[i] == 0)
            return false;
        graph.each_label_start_index.push_back(new_id_count);
...
bool sorted_edge_list_to_csr(const std::vector<std::tuple<uint64_t, uint64_t, EdgeData>> edge_list,
        ...
        LABEL_SUMMARY & ls, bool enable_edge_matching = false) {
    assert(graph.vertex_edgelist_start_opt.size() == 0, "vertex_edgelist_start_opt are not cleared");
    ...
    for (auto it = ls.unique_edges.begin(); it < ls.unique_edges.end(); it ++) {
        edge_type_found.insert(std::make_pair(*it, false));
    }
    ...
    while (count < edge_list.size()) {
        ...
        if (valid_edge(cur_source, cur_dest, cur_edge_data, new_source, new_dest,
                    graph, id_mapping, ls, enable_edge_matching, edge_type_found)) {
    ...
    //constrcut graph
    assert(graph.vertex_edgelist_start_index.size() == 0);
    assert(graph.edges.size() == 0);
    assert(graph.edge_data.size() == 0);
    size_t start_index = 0;
    for (size_t i = 0; i<graph.vertex_num; i++) {
        graph.vertex_edgelist_start_index.push_back(start_index);
        sort(new_edge_list[i].begin(), new_edge_list[i].end());

        size_t vertex_data_id = 0;
        for (size_t j=0; j<new_edge_list[i]; j++, start_index++) {
            Vertex dst = std::get<0>(new_edge_list[i][j]);
            graph.edges.push_back(dst);
            graph.edge_data.push_back(std::get<1>(new_edge_list[i][j]));

            while (dst >= graph.each_label_start_index[vertex_data_id+1]) {
                graph.start_pos_for_each_label.push_back(start_index);
                vertex_data_id++;
                assert(vertex_data_id < graph.each_label_start_index.size() - 1);
            }
        }
...
bool valid_edge(Vertex org_src, Vertex org_dst, EdgeData edge_data,
        ...
        LABEL_SUMMARY & ls, bool enable_edge_matching = false,
        std::map<std::tuple<VertexData, VertexData, EdgeData>, bool> & edge_type_found) {
    if (!id_map.to_new_id(org_src, new_src))
        return false;

    if (!id_map.to_new_id(org_dst, new_dst))
        return false;
