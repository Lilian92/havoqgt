...
public:
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
    ...
    // log2  
    int uniform_random_vertex_metadata = std::get<1>(alg_data);
    std::get<0>(alg_data)[vertex] = (uniform_random_vertex_metadata <= 0) ? \
        static_cast<VertexData>(ceil(log2(g.degree(vertex) + 1))) : static_cast<VertexData>(rand() % uniform_random_vertex_metadata);
...
template <typename TGraph, typename VertexMetaData, typename Vertex, 
  typename VertexData>
void vertex_data_db_degree(TGraph* g, VertexMetaData& vertex_metadata, int& uniform_random_vertex_metadata) {
  ...
  auto alg_data = std::forward_as_tuple(vertex_metadata, uniform_random_vertex_metadata);
